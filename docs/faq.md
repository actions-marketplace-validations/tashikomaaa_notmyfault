...
GitLab CI/CD, yes: see [GitLab CI/CD](gitlab.md). Forgejo and Gitea Actions too: see [Forgejo and Gitea Actions](forgejo.md). Jenkins, CircleCI and any other CI system with Node.js and git run it as well, without pull request comments: see [Any other CI system](any-ci.md). A dedicated integration only needs an implementation of `src/platform.ts`: contributions are welcome.
