...
function toResult(testcase, suite) {
  ...
  const seconds2 = Number(testcase.attrs.time);
  if (testcase.attrs.time?.trim() && Number.isFinite(seconds2) && seconds2 >= 0) result.duration = Math.round(seconds2 * 1e3);
  if (outcome !== "skipped") {
...
var VariablesIO = class {
  ...
  describeInputs(names2) {
    return `Variables ${names2.map(variable).join(" and ")}`;
  }
  setOutput(name, value) {
  ...
  appendSummary(markdown) {
    ...
    const text = this.redact(markdown) + EOL2;
    if (this.summaryStarted) appendFileSync2(file, text);
...
var GitLabClient = class {
  ...
  project;
  jobToken;
  async upsertComment(mergeRequest, marker, body, create) {
    const notes = `/projects/${this.project}/merge_requests/${mergeRequest}/notes`;
    const existing = (await this.list(`${notes}?per_page=100&sort=asc&order_by=created_at`)).find(
  ...
  async listIssues(label) {
    const items = await this.list(`/projects/${this.project}/issues?labels=${encodeURIComponent(label)}&state=all&per_page=100`);
  ...
  async changeOf(sha) {
    ...
    const merged = requests.filter((request2) => request2.state === "merged");
   ...
   */
  async request(method, path, body, redirects = 3) {
    const response = await fetch(`${this.apiUrl}${path}`, {
      method,
      redirect: "manual",
      headers: {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      const next = location ? samePage(new URL(location, `${this.apiUrl}${path}`).href, this.apiUrl) : void 0;
      if (!next || redirects === 0) {
...
function readGitLabContext(env) {
  const repository = env.CI_MERGE_REQUEST_PROJECT_PATH || required2(env, "CI_PROJECT_PATH");
  ...
  return {
    ...
    sha: required2(env, "CI_COMMIT_SHA"),
    // Merge request pipelines have no CI_COMMIT_BRANCH, tag pipelines neither.
    branch: env.CI_PIPELINE_SOURCE === "merge_request_event" ? void 0 : env.CI_COMMIT_BRANCH || void 0,
    runDescription: `pipeline ${env.CI_PIPELINE_ID || "local"}, job ${env.CI_JOB_ID || "local"}`,
    runUrl: env.CI_JOB_URL || void 0,
    workspace: env.CI_PROJECT_DIR ?? process.cwd(),
    tempDir: void 0,
    defaultBranch: env.CI_DEFAULT_BRANCH,
    defaultKey: env.CI_JOB_NAME ?? "job",
    pullRequest: Number.isInteger(mergeRequest) && mergeRequest > 0 ? { number: mergeRequest, fromFork: sourceProject !== void 0 && sourceProject !== env.CI_MERGE_REQUEST_PROJECT_ID } : void 0,
    defaultToken: env.CI_JOB_TOKEN || void 0
  };
}
...
var GitLabIO = class extends VariablesIO {
  ...
  annotation(level, message, properties) {
    ...
    this.issues.push({
      ...
      fingerprint: createHash3("sha256").update(`${path}:${properties.title ?? ""}`).digest("hex").slice(0, 32),
  ...
  finish() {
    ...
    writeFileSync2(this.path("NOTMYFAULT_CODE_QUALITY_FILE", "gl-code-quality-report.json"), `${report}
`);
    super.finish();
  }
};
